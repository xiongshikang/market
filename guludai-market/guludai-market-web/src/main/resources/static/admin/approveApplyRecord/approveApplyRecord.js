var approveApplyRecord = {
    baseUrl: "/market/web/approveApplyRecord",
    entity: "approveApplyRecord",
    tableId: "userTable",
    toolbarId: "toolbar",
    unique: "id",
    order: "asc",
...
approveApplyRecord.columns = function () {
    return [{
        checkbox: true
    }, {
        field: 'bossName',
...
approveApplyRecord.init = function () {
    approveApplyRecord.table = $('#' + approveApplyRecord.tableId).bootstrapTable({
        url: approveApplyRecord.baseUrl, //请求后台的URL（*）
        method: 'post', //请求方式（*）
        toolbar: '#' + approveApplyRecord.toolbarId, //工具按钮用哪个容器
        striped: true, //是否显示行间隔色
        cache: false, //是否使用缓存，默认为true，所以一般情况下需要设置一下这个属性（*）
        pagination: true, //是否显示分页（*）
        sortable: false, //是否启用排序
        sortOrder: approveApplyRecord.order, //排序方式
        //queryParams: approveApplyRecord.queryParams,//传递参数（*）
        ...
        uniqueId: approveApplyRecord.unique, //每一行的唯一标识，一般为主键列
        showToggle: true, //是否显示详细视图和列表视图的切换按钮
        cardView: false, //是否显示详细视图
        detailView: false, //是否显示父子表
        columns: approveApplyRecord.columns(),
...
approveApplyRecord.select = function (layerTips) {
    var rows = approveApplyRecord.table.bootstrapTable('getSelections');
    if (rows.length == 1) {
        approveApplyRecord.currentItem = rows[0];
        return true;
    } else {
        layerTips.msg("请选中一行");
        return false;
...
layui.use(['form', 'layedit', 'laydate'], function () {
    ...
    $('#btn_query').on('click', function () {
        approveApplyRecord.table.bootstrapTable('refresh');
    });
    //同意
    ...
    $('#btn_edit').on('click', function () {
        if (approveApplyRecord.select(layerTips)) {
            var applyCode = approveApplyRecord.currentItem.applyCode;
            doAjax(approveApplyRecord.baseUrl + '/nodeList', function(data) {
 				if (data.code==200) {	
...
function view(layer,form,result,userList,modelData){
	$.get(approveApplyRecord.entity+'/edit', null, function (form) {
         layer.open({
             type: 1,
             title: '节点信息',
             content: form,
