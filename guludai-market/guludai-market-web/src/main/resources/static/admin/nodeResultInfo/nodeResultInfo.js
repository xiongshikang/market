var nodeResultInfo = {
    baseUrl: "/market/web/nodeResultInfo",
    entity: "nodeResultInfo",
    tableId: "userTable",
    toolbarId: "toolbar",
    unique: "id",
    order: "asc",
...
nodeResultInfo.columns = function () {
    return [{
        checkbox: true
    }, {
        field: 'bossName',
...
nodeResultInfo.init = function () {
    nodeResultInfo.table = $('#' + nodeResultInfo.tableId).bootstrapTable({
        url: nodeResultInfo.baseUrl, //请求后台的URL（*）
        method: 'post', //请求方式（*）
        toolbar: '#' + nodeResultInfo.toolbarId, //工具按钮用哪个容器
        striped: true, //是否显示行间隔色
        cache: false, //是否使用缓存，默认为true，所以一般情况下需要设置一下这个属性（*）
        pagination: true, //是否显示分页（*）
        sortable: false, //是否启用排序
        sortOrder: nodeResultInfo.order, //排序方式
        //queryParams: nodeResultInfo.queryParams,//传递参数（*）
        ...
        uniqueId: nodeResultInfo.unique, //每一行的唯一标识，一般为主键列
        showToggle: true, //是否显示详细视图和列表视图的切换按钮
        cardView: false, //是否显示详细视图
        detailView: false, //是否显示父子表
        columns: nodeResultInfo.columns(),
...
nodeResultInfo.select = function (layerTips) {
    var rows = nodeResultInfo.table.bootstrapTable('getSelections');
    if (rows.length == 1) {
        nodeResultInfo.currentItem = rows[0];
        return true;
    } else {
        layerTips.msg("请选中一行");
        return false;
...
layui.use(['form', 'layedit', 'laydate'], function () {
    ...
    $('#btn_edit1').on('click', function () {
        ...
        nodeResultInfo.table.bootstrapTable('refresh');
    });
    //接受申请
   ...
   //双方签约
    ...
    $('#btn_edit').on('click', function () {
        if (nodeResultInfo.select(layerTips)) {
            var applyCode = nodeResultInfo.currentItem.applyCode;
            doAjax(nodeResultInfo.baseUrl + '/nodeList', function(data) {
 				if (data.code==200) {	
...
function view(layer,form,result,userList,modelData){
	$.get(nodeResultInfo.entity+'/edit', null, function (form) {
         layer.open({
             type: 1,
             title: '节点信息',
             content: form,
